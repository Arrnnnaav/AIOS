...
| # | Mutation | Must fail |
|---|---|---|
| 1 | Delete the `CurrentProcessId != target_pid` check | `test_returns_empty_when_focus_is_in_another_process` |
| 2 | `return element.CurrentAutomationId or "x"` | `test_reports_the_focused_controls_automation_id` |
| 3 | Delete the `if hwnd <= 0: return ""` guard | `test_returns_empty_for_a_dead_window_handle` |
